...
<style>
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .entry .del-btn svg { transform-origin: center; }
  ...
  container.innerHTML = errors.map((e, i) => {
    ...
    return `<div class="${cls}">
      ...
      <div class="actions">
        ...
        <button class="del-btn" onclick="deleteError(${e.id})" title="Delete entry"><svg viewBox="0 0 16 16" width="14" height="14" fill="currentColor" style="transform:scaleX(${1-(i/Math.max(errors.length-1,1))*0.8})"><path d="M5.5 2h5l.5.5V3H5v-.5zM3 4h10v.5l-.5.5H3.5L3 4.5zm1.5 1.5h1l.5 8h-1zm3 0h1l.5 8h-1zm3 0h1l-.5 8h-1zM4.5 2.5A1.5 1.5 0 016 1h4a1.5 1.5 0 011.5 1.5V3h2.5a.5.5 0 010 1H14l-.7 9.3a1.5 1.5 0 01-1.5 1.4H4.2a1.5 1.5 0 01-1.5-1.4L2 4h-.5a.5.5 0 010-1H4z"/></svg></button>
...
function formatTime(iso) {
  ...
  const mo = d.getMonth()+1, da = d.getDate(), y = d.getFullYear();
  let h = d.getHours(), m = d.getMinutes();
  const ampm = h >= 12 ? "PM" : "AM";
  h = h % 12 || 12;
  return mo + "/" + da + "/" + y + "-" + h + ":" + String(m).padStart(2,"0") + "-" + ampm;
}
...
async def handle_esp32_trigger(request):
    try:
        ...
        return web.json_response({"ok": False, "error": str(e)}, status=400)
    ts = datetime.now(timezone.utc).strftime("%m/%d/%Y-%I:%M-%p")
    from bridge import CAMERAS
