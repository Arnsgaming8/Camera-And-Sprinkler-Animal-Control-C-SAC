...
class MjpegCameraProvider(CameraProvider):
    name = "mjpeg"
    ...
    def __init__(self, config: dict, session: aiohttp.ClientSession | None = None):
        ...
        self._url = config.get("url") or config.get("mjpeg_url") or ""
    ...
    async def connect(self) -> bool:
        if not self._url:
            errors.log_error("mjpeg.connect", "No MJPEG URL provided")
            return False
        try:
            import cv2
            full_url = self._url
            ...
            if not self._cap.isOpened():
                errors.log_error("mjpeg.connect", "Failed to open MJPEG stream")
                return False
            self._connected = True
            return True
        except Exception as e:
            errors.log_error("mjpeg.connect", str(e), exc_info=True)
            return False
...
register("mjpeg", MjpegCameraProvider)
