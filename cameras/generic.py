...
class GenericCameraProvider(CameraProvider):
    name = "generic"
    ...
    def __init__(self, config: dict, session: aiohttp.ClientSession | None = None):
        ...
        self._url = config.get("url") or config.get("stream_url") or ""
    ...
    async def connect(self) -> bool:
        if not self._url:
            errors.log_error("generic_cam.connect", "No stream URL provided")
            return False
        try:
            import cv2
            full_url = self._url
            if self._username and self._password:
                ...
                if parsed.scheme in ("rtsp", "rtsps"):
                    ...
                    full_url = f"http://{self._username}:{self._password}@{parsed.hostname}:{parsed.port or 80}{parsed.path}"
            self._cap = cv2.VideoCapture(full_url)
            if not self._cap.isOpened():
                errors.log_error("generic_cam.connect", "Failed to open stream")
                return False
            self._connected = True
            return True
        except Exception as e:
            errors.log_error("generic_cam.connect", str(e), exc_info=True)
            return False
...
register("generic", GenericCameraProvider)
